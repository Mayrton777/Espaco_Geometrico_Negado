...
class Calcular_SIRN:
    def __init__(self, antena_analisada, azimute, elevacao, dados):
        ...
        if not indice.empty:
            ...
            self.lar_faixa = dados['DesignacaoEmissao'].iloc[indice[0]]
    ...
    def calcular_eipr(self, ganho, pot):
    ...
    def calcular_ruido(self):
        return -174 + 10 * log10(largura_canal_hz(self.lar_faixa)) + 7
    ...
    def calculo_interferencia(self, lat_movel, lon_movel, alt_movel):
        ...
        for i, row in self.df.iterrows():
            if row['Azimute'] != self.az and row['FreqTxMHz'] == self.freq and row['DesignacaoEmissao'] == self.lar_faixa:
                ...
                if distancia_para_movel <= 1000:#and (azimute_direcao_movel > -180 and azimute_direcao_movel < 180)
                    ...
                    distancia_diagonal_movel = mapa_antena_i.calc_hipotenusa((altura_i + altitude_antena_i) - (self.alt_movel + alt_movel), distancia_para_movel)
                    ...
                    eirp_antena = self.calcular_eipr(ganho_gaussiano, potencia_i)
                    # Calcula a perda do espaço livre dB
                    free_space = self.calcular_perda_no_espaco(distancia_diagonal_movel)
                    # Calcula o Sinal dBm
                    sinal = eirp_antena - free_space
                    s_i += dBm_to_mW(sinal)
    ...
    def calculo_SINR(self, distancia, azimute):
        ...
        altitude_movel = altitudeCalculator.calc_altitude(lat_movel, lon_movel)
        # Calcula a distancia do móvel até o ponto mais alto da antena
        distancia_diagonal = self.calcular_mapa_antena.calc_hipotenusa((self.alt + altitude_antena) - (self.alt_movel + altitude_movel), distancia)
        ...
        # Calcula o EIRP dBm/Hz
        eirp_antena = self.calcular_eipr(ganho_gaussiano, self.pot)
        # Calcula a perda do espaço livre dB
        free_space = self.calcular_perda_no_espaco(distancia)
        # Calcula o Ruido dBm
